...
-   `test_full_tls_handshake_simulation` для проверки того, что полное рукпоожатие (`client_hello`, `server_hello`, `server_key_exchange`, `finish_handshake`) отрабатывает корретно, то есть что одно сообщение цепочки корректно вызывает следующие и в финале они приводят к корректному установлению ключей, а значит, и к корректному установлению TLS-соединения.
    ...
    Видео прохождения тестов находится в файле `tests/test_running_screencast.mp4`.

# ПБ, ЦБ, Схемы

## Предположения безопасности

-   Система планирования заданий получает от оператора только аутентичные команды.

## Цели безопасности

-   При любых обстоятельствах обеспечить безопасную передачу сообщений между системой планирования заданий и связью.

## Политика архитектуры

![Политика архитектуры](images/scheme_original.png)

Если бы блок связи был повыщающим целостность, то всё общение между системой планирования и связью стало бы доверенным:
![Политика архитектуры с повышением целостности](images/scheme_if_comm_is_orange.png)
Но это отдельная задача, которая не входит в рамки текущего проекта.
